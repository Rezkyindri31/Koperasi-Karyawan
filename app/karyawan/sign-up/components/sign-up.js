    ...
    try {
      ...
      if (data?.errors) {
        ...
        Object.entries(data.errors).forEach(([field, msgs]) => {
          ...
          const key = field === "email" ? "emailLocal" : field; // map ke field kita
          serverErrors[key] = first;
        });
        setErrors(serverErrors);
      } else {
        setErrors({
          general: data?.message || "Pendaftaran gagal. Coba lagi.",
        });
      }
    }
  };
    ...
    <div className="flex min-h-screen">
      <div className="flex-1 flex flex-col justify-center px-10 py-12 bg-white">
        <h1 className="text-3xl font-bold mb-2">
          Buat Akun Koperasi Karyawan Kita
        </h1>
        <p className="text-gray-500 mb-6">
          Daftarkan diri Anda dan dapatkan Layanan Kami.
        </p>
        ...
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          <div className="relative w-full">
            ...
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 pr-2 text-gray-500"
              aria-label={
        ...
        <button
          onClick={() => router.push("/karyawan/sign-in")}
